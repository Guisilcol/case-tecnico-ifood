resource "aws_glue_catalog_table" "nyc_taxi_data_yellow" {
  database_name = aws_glue_catalog_database.bronze_db.name
  name          = "nyc_taxi_data_yellow"
  ...
  storage_descriptor {
    location      = "${var.bronze_db_s3_location}/nyc_taxi_data_yellow/"
    ...
    columns {
      name    = "vendorid"
      type    = "bigint"
      comment = "Identificador do fornecedor TPEP que forneceu o registro"
    }
